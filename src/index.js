...
async function downloadWarpDeb(channel, version) {
  if (process.platform !== 'linux') {
    throw new Error(`Only Linux runners are supported - the current platform is ${process.platform}`);
  }
  ...
  if (version === 'latest') {
    ...
    const response = await client.get(`https://app.warp.dev/download/cli?os=linux&package=deb&arch=${arch}&channel=${channel}`)
    ...
    if (response.message.statusCode === 302 || response.message.statusCode === 301) {
      ...
      throw new Error(`Expected redirect from ${redirectUrl}, got status ${response}`);
    }
